...
### Additional Note: Synthetic / Manmade Matrix Testing

To validate the entropy pipeline under controlled conditions, a synthetic Hi‑C matrix generator is provided:

**Script:** `Codes/Compute_entropy/Manmade_matrix.py`

This script builds an artificial Hi‑C interaction matrix with customizable intra‑ vs. inter‑chromosomal probabilities.  
It is useful for confirming algorithm behavior and benchmarking entropy outcomes.

It generates:
- A synthetic contact matrix
- Row‑normalized probability matrix \( P_{ij} \)
- Entropy results
- Heatmap: `synthetic_<mode>_heatmap.png`

Run it via:
```bash
python Manmade_matrix.py
```
